...
namespace neural {
	...
	class Tensor {
		public:
			using value_type = T;
			using matrix_type = Eigen::Matrix< value_type, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor >;
		public:
			Tensor() noexcept;
	...
	template< typename T >
	Tensor<T>::Tensor( std::size_t rows, std::size_t cols ) noexcept : m_mat( rows, cols ) {
		m_mat.setZero();
	}

	template< typename T >
	template< std::random_access_iterator It >
	Tensor<T>::Tensor( std::size_t rows, std::size_t cols, It begin, It end ) : m_mat( rows, cols ) {
		std::size_t const size = std::distance( begin, end );

		if( size != rows *cols ) {
			throw std::invalid_argument( "The number of elements in the range must be equal to the number of elements in the tensor" );
		}
	...
	template< typename T >
	Tensor<T>::Tensor( std::size_t rows, std::size_t cols, value_type value ) noexcept : m_mat( rows, cols ) {
	...
	template< typename T >
	typename Tensor<T>::value_type Tensor<T>::operator()( std::size_t row, std::size_t col ) {
		return m_mat( row, col );
	}

	template< typename T >
	typename Tensor<T>::value_type const Tensor<T>::operator()( std::size_t row, std::size_t col ) const {
		return m_mat( row, col );
	}

	template< typename T >
	Tensor<T> &Tensor<T>::transpose() noexcept 
  {
