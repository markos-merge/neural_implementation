                        ...
                        std::size_t                           batch_size = 1024 )
{
    ...
    for ( std::size_t start = 0; start < n; start += batch_size ) {
        ...
        for ( std::size_t i = 0; i < b; ++i )
            std::copy_n( images[start + i].data(), in_cols, batch_x.data() + i * in_cols );
        ...
        for ( std::size_t i = 0; i < b; ++i ) {
            ...
            std::size_t pred = static_cast<std::size_t>( std::max_element( row, row + n_cls ) - row );
            std::size_t gt   = 0;
            for ( std::size_t c = 0; c < n_cls; ++c )
                if ( labels[start + i]( 0, c ) > T( 0.5 ) ) { gt = c; break; }
            if ( pred == gt )
                ++correct;
        }
    }
