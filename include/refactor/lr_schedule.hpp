...
struct ConstantLR {
    T m_lr = T( 0.01 );
    T operator()( std::size_t /*epoch*/, std::size_t /*total*/ ) const { return m_lr; }
};

// ── Step decay ────────────────────────────────────────────────────────────────
// lr = lr_initial * gamma ^ (epoch / step_size)  (integer division)
template <typename T>
struct StepLR {
    T           m_lr_initial;
    T           m_gamma     = T( 0.1 );
    ...
    {
        return m_lr_initial * std::pow( m_gamma, static_cast<T>( epoch / m_step_size ) );
    }
};
...
struct CosineAnnealingLR {
    ...
    {
        T const p = total > 0
                        ? std::min( static_cast<T>( epoch ) / static_cast<T>( total ), T( 1 ) )
                        : T( 1 );
