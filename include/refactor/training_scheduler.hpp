...
//   - Applies m_lr_schedule at the start of each epoch to set
//   opt.m_learning_rate.
//   - After assembling each flat batch, calls m_batch_transform(batch_x,
//   batch_y)
//     (non-owning Tensor views) — use this for augmentation, normalization,
//     etc.
//   - Calls opt.step() for each batch and reports progress via an optional
//   callback.
//
// The optimizer is a pure gradient-update engine; the scheduler drives it.
//
// Usage (with augmentation):
//   using Opt   = MomentumSGDOptimizer<float, Cuda, CrossEntropyLoss<float,
//   Cuda>>; using Sched = TrainingScheduler<float, Cuda, Opt,
...
template <typename T, typename Device, typename Optimizer,
...
class TrainingScheduler
{
	public:
		...
		using ProgressCallback = std::function<bool(
		    std::size_t epoch, std::size_t total_epochs, std::size_t batch,
		    std::size_t total_batches, T loss, T lr )>;

		void train( Optimizer &opt, SequentialNN2<T, Device> &nn,
	...
	private:
		static void fill_batch( std::vector<neural::Tensor<T>> const &inputs,
		                        ...
		                        std::vector<T> &batch_x, std::vector<T> &batch_y );
};

// ---------------------------------------------------------------------------

template <typename T, typename Device, typename Optimizer, typename LRSchedule,
          typename BatchTransform>
void TrainingScheduler<T, Device, Optimizer, LRSchedule, BatchTransform>::
    fill_batch( std::vector<neural::Tensor<T>> const &inputs,
                std::vector<neural::Tensor<T>> const &targets,
                std::vector<std::size_t> const &order, std::size_t start,
                std::size_t count, std::vector<T> &batch_x,
...
void TrainingScheduler<T, Device, Optimizer, LRSchedule, BatchTransform>::train(
    Optimizer &opt, SequentialNN2<T, Device> &nn,
    std::vector<neural::Tensor<T>> const &inputs,
    std::vector<neural::Tensor<T>> const &targets, ProgressCallback cb )
{
	...
	for ( std::size_t epoch = 0; epoch < m_epochs; ++epoch ) {
		...
		for ( std::size_t bi = 0; bi < batches_per_epoch; ++bi ) {
			...
			T const loss =
			    opt.step( nn, batch_x.data(), count, in_cols, batch_y.data() );
